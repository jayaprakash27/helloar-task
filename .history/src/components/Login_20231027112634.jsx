...
function Login() {
  ...
  const handleVerifyOTP = () => {
    const apiEndpoint = 'https://dev.api.goongoonalo.com/v1/auth/verify_otp';
    const requestBody = {
      phoneNumber: phoneNumber,
      requestId: reqId,
      otp: otp.join(""),
    ...
    fetch(apiEndpoint, {
      ...
      if (otp.join("") === '5678') {
        dispatch({
          type: "LOGIN",
          userToken: requestBody,
        });
        localStorage.setItem("signedInUser", requestBody);
      }
  };
  ...
  return (
    <div className="login-page">
      ...
      <div className=" login-box flex flex-col w-80 gap-2 items-center justify-center">
        ...
        <p onClick={clearOTPFields} className=" text-gray-700 underline cursor-pointer"> Resend OTP </p>
        <p onClick={anotherNum} className=" text-gray-700 underline cursor-pointer"> Use another number </p>
      </div>
      }
    </div>
  );
